...
simulator (results/long_sim). Produces (into plots/, continuing the 01-19 sequence):
  - 20_business_latency_cost.png  : two panels (latency SLA | annualized cost)
  - 21_business_cost_stability.png: cost($)-vs-stability frontier
  - 22_business_stability.png     : scaling actions (operational churn)
  - summary_business_case.csv     : the underlying table
and prints the two-regime verdict.

Cost model (identical for every controller in results/long_sim):
...
RESULTS_DIR = os.path.join("results", "long_sim")
OUT_DIR = "plots"
...
def fig_latency_cost(rep):
    ...
    _save(fig, "20_business_latency_cost")
...
def fig_cost_stability(s, rep):
    ...
    fig.tight_layout()
    _save(fig, "21_business_cost_stability")
...
def fig_stability(rep):
    ...
    fig.tight_layout()
    _save(fig, "22_business_stability")
...
def main():
    ...
    s.sort_values("cost_usd").to_csv(os.path.join(OUT_DIR, "summary_business_case.csv"), index=False)
    fig_latency_cost(rep)
    fig_cost_stability(s, rep)
    fig_stability(rep)
    two_regime_verdict(s, rep)
    print(f"\nSummary table -> {OUT_DIR}/summary_business_case.csv")

...
Reframes the long-simulation results (hardened simulator, `results_long_v2/`, Alibaba
...
| File | Shows |
|---|---|
| `fig_business_latency_cost.png` | **The headline two-panel:** latency-SLA (left) and annualized cost (right), one representative config per model + baselines. |
| `fig_business_cost_stability.png` | Cost vs. scaling-churn at matched SLA (≥99%) — where the real differentiation lives. |
| `fig_business_stability.png` | Scaling actions over 24 h — LLMs are 6–20× more stable than HPA/KEDA. |
| `business_case_summary.csv` | Full per-config table (SLA, latency, cost vCPU + $, scale events). |
...
- Cost is comparable **within** `results_long_v2` only (same 0.25 vCPU basis); do not
  compare $ against the pre-rework `results_long/`.
- 25 of 28 runs included; llama-70b history_5/cot and scout/cot were still completing —
  rerun the script when they finish to fold them in (it auto-includes complete runs only).

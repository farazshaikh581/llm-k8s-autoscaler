...
def build_system_prompt(max_r: int) -> str:
    return (
        ...
        f"Max replicas: {max_r}. SLA: P90 latency < 200ms. "
        f'Respond with ONLY: {{"replicas": N}} where N is 1-{max_r}.'
    )


SYSTEM_PROMPT = build_system_prompt(REPLICA_MAX)


def set_replica_cap(max_r: int):
    """Set the schedulable replica ceiling (and reflect it in the LLM prompt)."""
    global REPLICA_MAX, SYSTEM_PROMPT
    REPLICA_MAX = max_r
    SYSTEM_PROMPT = build_system_prompt(max_r)
...
def run(args):
    ...
    svc_name = wl["service"]
    if args.deployment_override:
        deployment = args.deployment_override
        svc_name = args.deployment_override

    svc_ip = kubectl("get", "svc", svc_name,
    ...
    with open(out_file, mode, newline="") as f:
        ...
        for step in range(start_step, len(trace)):
            # Intentionally indexed by loop iteration, not wall-clock time: when a
            # model's own inference latency exceeds `interval`, this step counter
            # falls behind the load generator's real-time trace position, so the
            # LLM reacts to an increasingly stale rps_target. This is the real-cluster
            # manifestation of the reactivity tradeoff (slow models can't keep pace
            # with real-time load) — keep as-is, do not resync to wall-clock.
...
def main():
    ...
    parser.add_argument("--max-replicas", type=int, default=20,
                        help="schedulable replica ceiling (fits the real cluster)")
